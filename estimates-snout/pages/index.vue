<template>
    <div class="container">
      ...
      <form @submit.prevent="submitTasks">
        <button type="button" @click="addTask" class="add-button">Añadir tarea</button>
        <button type="submit" class="submit-button">Enviar</button>
        <div class="task-list">
          <div v-for="(task, index) in tasks" :key="index" class="task-form">
            <button type="button" @click="showDeleteModal(index)" class="delete-button">Eliminar</button>
            <div class="task-inputs">
              <input v-model="task.title" placeholder="Título de la tarea" required />
              <textarea v-model="task.description" placeholder="Descripción (opcional)"></textarea>
            </div>
          </div>
        </div>
      </form>
  ...
  .add-button:hover, .submit-button:hover, .delete-button:hover {
    opacity: 0.9;
  }
